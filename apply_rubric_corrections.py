...
def apply_correction(q, lv, ans):
    ...
    if q == 1:
        ...
        # Q5 – "מה היית רוצה ללמוד?" (עניין וסקרנות – SDT)
        # L3→L2: תשובות חזרה-על-העבר בלי סקרנות ממוקדת
        if lv == 3 and re.search(
            ...
            return 2, 'מחוון L2: חזרה על אסטרטגיה קודמת ללא ביטוי סקרנות ממוקדת'
        # L2→L3: מציין נושא ספציפי שמעניין אותו
        if lv == 2 and re.search(
            ...
            return 3, 'מחוון L3: ציון נושא ספציפי שמעניין את התלמיד – סקרנות ממוקדת'
        # L3→L4: למידה מעשית – ניסויים, מחקר, יישום
        if lv == 3 and re.search(
            r'(ניסוי|לחקור|חקירה|מחקר|יישום|להתנסות|ניסיון\s*מעשי|'
            r'לנסות\s*בפועל|לעשות\s*בעצמ)', a
        ):
            return 4, 'מחוון L4: למידה פרקטית ויישומית – ניסויים / חקירה עצמית'
        ...
        # Q7 – "האם תלך/י לשיחה עם המורה?" (פעלנות מול המורה)
        # L1→L2: אומר "כן אלך" אבל אין מה לשאול – ציות פסיבי לא הימנעות
        if lv == 1 and re.search(r'^כן', a) and not re.search(
            r'(לא\s*רוצ|לא\s*ארצ|לא\s*אוהב|מביך|בושה|מפחיד|'
            r'לא\s*נוח|לא\s*ארצה\s*לדבר)', a
        ):
            return 2, 'מחוון L2: מסכים ללכת (ציות פסיבי) ללא הימנעות פעילה'
        # L2→L4: מבקש כלים לשיפור / מבחן חוזר / מועד ב'
        if lv == 2 and re.search(
            ...
            return 4, 'מחוון L4: חיפוש עזרה אקטיבי – בקשת כלים / מועד ב\' / שיפור מכוון'
        # L2→L3: שואל על טעויות ספציפיות לצורך הבנה
        if lv == 2 and re.search(r'(במה\s*טעיתי|הטעויות\s*שלי|מה\s*(לא\s*)?עשיתי\s*(לא\s*)?נכון)', a):
            if not re.search(r'(ואיך|ולשפר|כדי\s*להשתפר|שיפור)', a):
                return 3, 'מחוון L3: שואל על הטעויות הספציפיות – סקרנות בסיסית'
        # L3→L4: שואל על טעויות + איך להשתפר
        if lv == 3 and re.search(
            r'(במה\s*טעיתי|הטעויות\s*שלי|מה\s*(לא\s*)?עשיתי\s*(לא\s*)?נכון)', a
        ) and re.search(r'(ואיך|ולשפר|כדי\s*להשתפר|שיפור|פעם\s*הבאה|לפעם)', a):
            return 4, 'מחוון L4: טעויות + כוונת שיפור מוצהרת – חיפוש עזרה אקטיבי'

    elif q == 8:
        # Q8 – "מה תרצה/י לשאול?" (שאלות לשיחה – מטקוגניציה)
        # L1→L2: פנייה מפורשת לגורם עזרה חיצוני (לא "אלמד" סתמי)
        if lv == 1 and re.search(
            r'(מורה|הורים|אבא|אמא|חבר\b|חברה\b|אפנה|יפנה)', a
        ) and not re.search(r'(לא\s*(יודע|רוצ)|כלום|שום)', a):
            return 2, 'מחוון L2: פנייה מפורשת לגורם עזרה חיצוני – יש אסטרטגיה בסיסית'
        # L2→L3: מאבחן מה לא הבין לפני שמבקש עזרה
        if lv == 2 and re.search(
            r'(להבין\s*(מה|איפה|למה|קודם)|מה\s*לא\s*(הבנתי|ידעתי)|'
            r'אאבחן|אזהה|לאתר\s*את\s*הטעות)', a
        ):
            return 3, 'מחוון L3: אבחון עצמי של פערי הבנה לפני פנייה לעזרה'
        # L2→L4: מבקש מועד ב' / עזרה ממספר מקורות
        if lv == 2 and re.search(r'(מועד\s*ב|מבחן\s*חוזר)', a):
            return 4, "מחוון L4: בקשת מועד ב' – פנייה אקטיבית לשיפור ציון"
